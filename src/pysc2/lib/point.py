...
class Rect(collections.namedtuple("Rect", ["t", "l", "b", "r"])):
    ...
    def __new__(cls, *args):
        ...
        if len(args) == 2:
            ...
            return super(Rect, cls).__new__(
                cls, min(p1.y, p2.y), min(p1.x, p2.x), max(p1.y, p2.y), max(p1.x, p2.x)
            )
        if len(args) == 4:
    ...
    def contains_point(self, pt):
        ...
        return self.l < pt.x and self.r > pt.x and self.t < pt.y and self.b > pt.y

    def contains_circle(self, pt, radius):
        """Is the circle completely inside this rect?"""
        return (
            self.l < pt.x - radius
            and self.r > pt.x + radius
            and self.t < pt.y - radius
            and self.b > pt.y + radius
        )
    ...
    def intersects_circle(self, pt, radius):
        ...
        if (
            circle_center.x > rect_corner.x + radius
            or circle_center.y > rect_corner.y + radius
        ):
            return False

        # Is the circle center inside the rect or near one of the edges?
        if circle_center.x <= rect_corner.x or circle_center.y <= rect_corner.y:
            return True
