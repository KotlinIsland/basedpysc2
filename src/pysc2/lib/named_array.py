...
class NamedNumpyArray(np.ndarray):
            ...
            if o is None:
                ...
                if isinstance(o, enum.EnumMeta):
                    ...
                    try:
                        ...
                        raise ValueError(
                            "Bad names. Must be None, a list of strings, "
        ...
        if isinstance(obj, np.ndarray):  # If this is a view, index the names too.
            ...
            for i, index in enumerate(indices):
                if isinstance(index, numbers.Integral):
                    ...
                    names = sorted(
                        self._index_names[dim].items(), key=lambda item: item[1]
                    )
            ...
            if len(obj._index_names) != len(obj.shape):
                raise IndexError(
                    "Names don't match object shape: %s != %s"
    ...
    def __repr__(self):
        ...
        for dim_names in self._index_names:
            if dim_names:
                dim_names = [
                    n for n, _ in sorted(dim_names.items(), key=lambda item: item[1])
                ]
                if len(dim_names) > 11:
        ...
        # ["NamedNumpyArray", "[1, 3, 6]", ", dtype=int32"]
        matches = re.findall(
            r"^(\w+)\(([\d\., \n\[\]]*)(,\s+\w+=.+)?\)$", np.array_repr(self)
        )[0]
        space = "\n               " if matches[2] and matches[2][1] == "\n" else ""
        return "%s(%s,%s %s%s)" % (matches[0], matches[1], space, names, matches[2])

    def __reduce__(self):
        # Support pickling: https://stackoverflow.com/a/26599346
        state = super(
            NamedNumpyArray, self
        ).__reduce__()  # pytype: disable=attribute-error
    ...
    def __setstate__(self, state):
        ...
        super(NamedNumpyArray, self).__setstate__(
            state[0:-1]
