...
#include "src/pysc2/env/converter/cc/visual_converter.h"
...
#include "src/pysc2/env/converter/cc/visual_actions.h"
...
dm_env_rpc::v1::Tensor AvailableActions(const SC2APIProtocol::Observation& obs,
  ...
  for (const auto& available_ability : obs.abilities()) {
    ...
    CHECK(found_applicable)
        << "Failed to find applicable action for " << available_ability.ShortDebugString();
  }
